...
// database connection
$conn = new mysqli('localhost', 'root','','pfi');
if($conn->connect_error){
    die('connection Failed : '.$conn->connect_error);
}else{
    $stmt = $conn->prepare("insert into school_info(schools_Name, Coordinator_Name, Schools_Email, Coordinator_Email, school_phone_no, Coordinators_Phone, Schools_Address)
    values(?,?,?,?,?,?,?)");
